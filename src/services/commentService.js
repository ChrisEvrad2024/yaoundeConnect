    ...
    static async updateComment(id, content, userId, userRole) {
        ...
        try {
            ...
            const canEdit = comment.user_id === userId ||
                ['admin', 'superadmin', 'moderateur'].includes(userRole);
    ...
    static async getCommentStats(poiId) {
        const stats = await Comment.findOne({
            ...
            attributes: [
                ...
                [sequelize.fn('COUNT', sequelize.literal('CASE WHEN parent_id IS NULL THEN 1 END')), 'root_comments'],
                [sequelize.fn('COUNT', sequelize.literal('CASE WHEN parent_id IS NOT NULL THEN 1 END')), 'replies'],
