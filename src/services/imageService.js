...
class ImageService {
  ...
  static async processUploadedImage(tempPath, finalDir, baseFilename) {
    try {
      ...
      const processedImage = await this.processImage(tempPath, finalDir, finalFilename);

      // Générer les thumbnails
      const thumbnails = await this.generateThumbnails(processedImage.path, finalFilename);
